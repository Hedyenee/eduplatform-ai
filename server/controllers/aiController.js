...
// Generate content from a free-form prompt using Gemini
...
exports.analyzeReviews = asyncHandler(async (req, res) => {
  ...
  const reviewsText = reviews
    .map((review, index) => `Review ${index + 1} (${review.rating}/5): "${review.comment || 'Pas de commentaire'}"`)
    .join('\n\n');
...
[La note moyenne est ${average.toFixed(2)}/5]
...
// @access Private
exports.generateCourseDescription = asyncHandler(async (req, res) => {
...
// @access Public
exports.suggestSimilarCourses = asyncHandler(async (req, res) => {
  const { courseId } = req.params;
...
exports.getPlatformInsights = asyncHandler(async (req, res) => {
  ...
  allReviews.forEach((review) => {
    const courseTitle = review.course?.title || 'Cours inconnu';
